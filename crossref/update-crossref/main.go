...
func main() {
	...
	request := &crossref.UpdateCrossRefRequest{
	...
	crossrefs, err := client.UpdateCrossRef(ctx, request)
	if nil != err {
		log.Fatalf("UpdateCrossRef: %s", err)
	}

	log.Println("Touched crossrefs ----------------------------------------------------")
	for _, crossref := range crossrefs.GetCrossrefs() {
